    ...
    for line in raw:
        ...
        if begin_body:
            ...
            if line.startswith("\t") or line.startswith(" "):
                ...
                original_message_id = re.search(r"Message-ID: *<([^>]*)>", line).group(
                    1
                )
                ...
                date = re.sub(
                    r"\(\w\wT\)", "", date
                ).strip()  # dateparser doesn't like eg. (PST)
                parsed_date = dateparser.parse(date)
                if parsed_date.year < 1985 or parsed_date.year > 2005:
        ...
        for fi in glob.iglob(f"{root}/**/*.", recursive=True):
            ...
            with open(fi) as infi:
                try:
                    ...
                    doc = parse_email(lines, username, folder, infi.name)
                    if doc:
                        docs.append(doc)
                    else:
                        print(f'Skipping {infi.name}')
                except UnicodeDecodeError:
                    print(f"Unable to read file {fi} as unicode")
                    with open(fi, "rb") as raw_binary:
                        encoding = chardet.detect(raw_binary.read())
                        print(
                            f'Detected encoding {encoding["encoding"]} with confidence {encoding["confidence"]}'
                        )
...
class ArgParser:
    def __init__(self):
        ...
        self.parser.add_argument(
            "-t", "--tarfile", help="Path to .tar.gz file to unzip"
        )
        self.parser.add_argument(
            "-d",
            "--directory",
            help="Output dir if -t specified, else input data dir"
